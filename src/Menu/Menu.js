...
// Load small menu images (Webpack require.context)
const menuSmImages = (() => {
  const m = {};
  try {
    const req = require.context("../img/menu/sm", false, /\.(png|jpe?g|svg)$/);
    req.keys().forEach((k) => (m[k.replace("./", "")] = req(k)));
  } catch (e) {
    // ok if not available in this build
  }
  return m;
})();
...
const Menu = () => {
  ...
  return (
    <section className="menu-bg">
      <img alt="About Background" className="h-200 menubg absolute left-0 top-0 -z-10 w-full object-cover" src="img/menubg.svg" onError={(e) => (e.currentTarget.src = placeholder)} />

      <div className="relative h-[230px]">
        <div className="absolute inset-0">
          <img src="/img/menu-banner.jpg" alt="Menu Banner" className="h-full w-full object-cover opacity-50" onError={(e) => (e.currentTarget.src = placeholder)} />
        </div>
        <div className="relative z-10 flex h-full flex-col items-center justify-center text-white">
          <h1 className="animate__animated animate__zoomIn mb-4 text-2xl font-bold md:text-4xl">Trending on the Menu</h1>
          <h2 className="quiklinks animate__animated animate__zoomIn text-3xl font-bold md:text-5xl">Most Loved at GreeNox</h2>
        </div>
      </div>
        ...
        <aside className="custom-scrollbar sticky top-20 hidden w-80 flex-col gap-4 overflow-y-auto rounded-lg bg-white shadow-lg lg:flex">
          <div className="sticky top-20 bg-white rounded-lg p-4 h-[100vh] overflow-y-auto">
            <div className="flex items-center gap-3 mb-4">
              <img src="/img/logo.png" alt="logo" className="h-10 w-10 rounded" onError={(e) => (e.currentTarget.src = placeholder)} />
              <div>
            ...
            <nav className="flex flex-col gap-2">
              {categories.map((cat) => {
                ...
                return (
                  ...
                  >
                    <img src={resolveItemImg(cat.image48Id || cat.image || "")} alt={cat.name} className="h-12 w-12 rounded-md object-cover flex-none" onError={(e) => (e.currentTarget.src = placeholder)} />
          ...
          <div className="bg-white rounded-lg p-4 mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex items-center gap-3 relative">
              <div className="relative">
                <button type="button" onClick={() => setOpenFilter((s) => !s)} className="px-4 py-2 bg-white border rounded text-sm inline-flex items-center gap-2">
                ...
                {openFilter && (
                  <div className="absolute left-0 left60 mt-2 w-40 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 z-30">
                    <div className="py-1">
                      <button onClick={() => { setTypeFilter("all"); setOpenFilter(false); }} className={`w-full text-left px-4 py-2 text-sm ${typeFilter === "all" ? "bg-gray-100" : "hover:bg-gray-50"}`}>All</button>
                      <button onClick={() => { setTypeFilter("veg"); setOpenFilter(false); }} className={`w-full text-left px-4 py-2 text-sm ${typeFilter === "veg" ? "bg-gray-100" : "hover:bg-gray-50"}`}>Veg</button>
                      <button onClick={() => { setTypeFilter("nonveg"); setOpenFilter(false); }} className={`w-full text-left px-4 py-2 text-sm ${typeFilter === "nonveg" ? "bg-gray-100" : "hover:bg-gray-50"}`}>Non‑Veg</button>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {activeItems.length === 0 ? (
              ...
              activeItems.map((item, idx) => {
                ...
                return (
                  <Link key={item.id ?? idx} to={`/menu/${catId}/${itemId}`} className="group bg-white rounded-lg p-4 flex flex-col justify-between hover:shadow-md transition">
                    <div className="flex gap-4">
                      <div className="flex-none">
                        <img src={resolveItemImg(item.img || item.imgFile)} alt={item.name} className="h-28 w-28 rounded-md object-cover" onError={(e) => (e.currentTarget.src = placeholder)} />
                      </div>

                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between">
                          <h4 className="text-lg font-semibold text-gray-900 line-clamp-1">{item.name}</h4>
                          <span className="ml-2">{item.type === "nonveg" ? <img src="/img/non-veg.svg" alt="nonveg" className="h-5 w-5" onError={(e) => (e.currentTarget.src = placeholder)} /> : <img src="/img/veg.svg" alt="veg" className="h-5 w-5" onError={(e) => (e.currentTarget.src = placeholder)} />}</span>
                        </div>
                        <p className="mt-2 text-sm text-gray-600 line-clamp-2">{item.description}</p>
                        <div className="mt-4"><span className="text-base font-semibold text-green-600">₹{item.price || "—"}</span></div>
                      </div>
                    </div>
                  </Link>
                );
